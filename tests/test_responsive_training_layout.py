...
class ResponsiveTrainingLayoutTests(unittest.TestCase):
    ...
    def test_training_photo_is_smaller(self):
        self.assertIn("MAIN_PHOTO_WIDTH = 300", self.source)
    ...
    def test_question_and_cta_live_with_photo(self):
        photo = self.source.index('st.image(student["photo_path"], width=MAIN_PHOTO_WIDTH)')
        question = self.source.index('st.markdown("### Quel est son nom ?")', photo)
        cta = self.source.index('"👀 Afficher le nom"', question)
        analytics = self.source.index("with analytics:", cta)
